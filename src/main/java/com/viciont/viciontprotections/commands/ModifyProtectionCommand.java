...
public class ModifyProtectionCommand implements CommandExecutor, TabCompleter {

    private final ViciontProtections plugin;
    private final ProtectionManager protectionManager;

    public ModifyProtectionCommand(ViciontProtections plugin, ProtectionManager protectionManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("viciontprotections.admin.mod")) {
            sender.sendMessage(plugin.formatMessage(plugin.getConfig().getString("messages.no_permission")));
            return true;
        }

        // Verificar mínimo de argumentos (nombre protección, acción y jugador)
        if (args.length < 3) {
        ...
        Protection protection = protectionManager.getProtectionByNameWithRefresh(protectionName);
        ...
        if (action.equalsIgnoreCase("add")) {
            ...
            if (protection.getMembers().contains(targetPlayer.getUniqueId())) {
                sender.sendMessage(plugin.formatMessage(plugin.getConfig().getString("messages.player_already_member")));
                return true;
            }
            ...
            if (!protection.getMembers().contains(targetPlayer.getUniqueId())) {
                sender.sendMessage(plugin.formatMessage(plugin.getConfig().getString("messages.player_not_member")));
                return true;
            }
